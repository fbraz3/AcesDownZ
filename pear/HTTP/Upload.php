...
class HTTP_Upload extends HTTP_Upload_Error
{
    ...
    function &_buildFiles()
    {
		...
		//Expressao regular pra separar a extensao do arquivo
		preg_match_all('/(.+)\.(.+)/s', $value['name'], $this->conteudo);
		$this->conteudo = array_reverse($this->conteudo);
		$this->ext = ".".$this->conteudo[0][0];
		$this->part_name = $this->conteudo[1][0];

		//Muda o nome do arquivo
		while(file_exists('../'.loadconfig('caminho_arquivos').$name)){
			$this->numero = $this->numero+1;
			$name = $this->part_name.".".$this->numero.$this->ext;	
		}
		//=======================================
